...
export const addMember = async(req,res)=>{
    try{
        ...
        //fetch workspace
        const workaspace = await prisma.workspace.findUnique({where:{id:workspaceId},include:{members:true}})
        if(!workaspace){
            return res.status(404).json({message:"Workspace not found"})
        }
        //check if user is admin of workspace
        if(!workaspace.members.find((member)=>member.userId === userId && member.role === "ADMIN")){
